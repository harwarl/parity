...
Originally these were separate deployables — `gauge-engine` (lib), `gauge-market`, `gauge-carder` — plus the HTTP surface (`gauge-api`) and push (`gauge-notif`) as two more. They're now **one deployable**, `gauge-api`, with each as an internal module (`mod engine`, `mod market`, `mod carder`) rather than separate processes/crates talking over Redis/HTTP. `gauge-notif` had no real logic to fold in (always just `println!("gauge-notif")`) — its future home is `mod notif` here, once there's an actual implementation. `gauge-exec` is the one deliberate exception, kept as its own service — see §5.
...
```
  ...
  Depth     ───────► │   calls mod engine per tick)                    │
  Calendar  ───────► │        │                                        │
...
`mod market` and `mod carder` still talk to each other over Redis Streams (not a plain function call) even though they're in the same process now — that keeps the option open to split them back out into separate deployables later (e.g. for independent scaling) without a rewrite, and it's what already made the merge itself a low-risk move: no internal data-flow logic changed, only how the modules are packaged and deployed. `mod engine` stays a pure, I/O-free module for the same reason it was a separate lib crate before — see 2.2.

---

## 2. Services

### 2.1 `gauge-exec` — MCP-only execution boundary (bin)
...
### 2.2 `gauge-api` — engine, ingest, user-plane fan-out, and the HTTP surface (bin)

One deployable, four internal pieces (`mod engine`, `mod market`, `mod carder`, `mod routes`) plus a couple of background tasks tying them together. This used to be four separate crates; see §1 for why they're one now and what's preserved from the split.

**`mod engine`** — pure math, no I/O. Given `RhjQuote` + `ChainlinkQuote` + `DepthSnapshot` + `Session` + `HaircutParams`, returns a `BasisTick` and a `Decision` (`CardEligible` / `Skip(code)` / `Halt(reason)`).
...
No retries, no clocks, no persistence, no knowledge of users — still true as a module, not just as a former crate. If a change needs any of those, it belongs in `mod market` or `mod carder`, not here.

**`mod market`** — owns the RHJ client, Chainlink client, depth client, and (eventually) the corporate-actions calendar. Calls `engine::evaluate()` per symbol per tick. Runs the halt state machine (section 6 of the product doc): multiplier jump or `oraclePaused` → halt; stale `updatedAt` → no live cards; zero depth → shrink/halt live; resumes manually or after N clean ticks. Publishes `BasisTick`s to Redis Streams.
...
**Push notifications** (`gauge-notif`, formerly its own crate) don't have a module here yet — there was never any real logic to move over. When it's built, it's `mod notif`: deliberately dumb, payload `card_id` only, client fetches the actual card from `mod routes`. No sensitive data would ever transit it, which is intentional — it can run with much weaker trust guarantees than everything else, whether or not it ends up living in this same binary.
...
2. Calls `engine::evaluate()` → `BasisTick` + `Decision`.
...
5. Push (`mod notif`, once built) sends `{card_id}` to relevant users.
...
- **`gauge-exec` is the sole holder of anything sensitive, and stays its own deployable because of it.** This is the one service boundary in the whole system that's about security, not convenience — merging `gauge-engine`/`gauge-market`/`gauge-carder`/`gauge-notif` into `gauge-api` was a safe simplification precisely because none of them ever held credentials; doing the same to `gauge-exec` would put real trading credentials in the same process and crash domain as the public-facing HTTP surface. Every other module — including `mod routes`, which talks to the frontend — should be buildable and testable with zero knowledge of execution credentials.
- **`mod engine` has zero trust surface by design** — it's pure functions, so its correctness is a testing problem, not a security problem. Being a module instead of a separate crate doesn't change that; it still has no I/O.
...
`gauge-api`'s `mod engine` + `mod market` (ingest + halt controller) + public tape. Fixtures pass the split/dividend/jump cases. No accounts, no cards, no `mod carder` loop running, no `gauge-exec`.

**Days 22–45 — prove the fan-out, paper only**
`mod carder` (cards, paper ledger) + `mod routes` + push. Skip-code histogram live and monitored. Zero live keys exist anywhere in the system — `gauge-exec` is stubbed or doesn't exist yet.
...
- **Push notifications**: `mod notif` doesn't exist yet — no payload delivery mechanism (APNs/FCM/web push) has been chosen.
