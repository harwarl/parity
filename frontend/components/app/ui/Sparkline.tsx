...
import { sparklineFrom } from "@/lib/gauge/series";
...
/** 112 × 28, last 24 nets ending on the live net, dashed floor. */
export function Sparkline({ row, values }: { row: WatchRow; values: number[] }) {
  const s = sparklineFrom(values);
  const c = netColor(row);
  return (
    <svg viewBox="0 0 112 28" width="112" height="28" role="img" aria-label={`${row.sym} net, recent ticks, now ${row.net.toFixed(1)} bps`}>
