...
const codes = [
  ...
  { code: "THIN", when: "Book depth cannot fill the clip without moving the price past the buffer." },
    ...
    >
      ...
      <div className="grid gap-x-12 gap-y-8 sm:grid-cols-2">
        {codes.map((c, i) => (
          ...
          >
            <Reveal scaleFrom={1.22} delay={i * 80} className="h-fit shrink-0">
              <span className="tnum inline-flex -rotate-3 items-center rounded-sm border border-halt/50 px-2 py-1 text-[11px] font-medium tracking-[0.18em] text-halt">
                {c.code}
              </span>
            </Reveal>
      ...
      <div className="mt-12 flex flex-col gap-4 border-t border-halt/40 pt-6 sm:flex-row sm:items-start">
        <span className="tnum inline-flex h-fit w-fit shrink-0 items-center gap-2 rounded-sm border border-halt/60 px-2 py-1 text-[11px] font-medium tracking-[0.18em] text-halt">
